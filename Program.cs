    ...
    class Program
    {
        ...
        static void ReadMnistDatabase(bool training, bool testing, out List<(MArray, MArray)> examples, out List<(MArray, int)> tests)
        {
            ...
            if (training)
            {
                for (int i = 0; i < 60000; i++)
                {
                    ...
                    for (int n = 0; n < 28 * 28; n++)
                        arr.Values[n] = trainImageData[16 + i * 28 * 28 + n] / 255d;
            ...
            if (testing)
            {
                for (int i = 0; i < 10000; i++)
                {
                    ...
                    for (int n = 0; n < 28 * 28; n++)
                        arr.Values[n] = testImageData[16 + i * 28 * 28 + n] / 255d;
        ...
        static MArray ReadTestFile(string path)
        {
            ...
            for (int y = 4; y < 24; y++)
            {
                for (int x = 4; x < 24; x++, i++)
                {
                    ...
                    bmpInput[x, y] = 1 - (pixel.R + pixel.G + pixel.B) / 768d;
                }
            }
        ...
        static void Main(string[] args)
        {
            ...
            if (args.Length > 0)
            {
                ...
                network.StochasticGradientDescent(epochs, learningRate, batchSize, examples.ToArray());
